...
/// </summary>
internal sealed class BlazorVirtualizePrefixSumTree
{
    ...
    public BlazorVirtualizePrefixSumTree(int count, double defaultValue)
    {
        _count = count;
    ...
    /// <summary>Number of items tracked by the tree.</summary>
    public int Count => _count;

    /// <summary>Sum of every item size (the full scrollable extent).</summary>
    public double Total => _total;

    /// <summary>Resizes the tree to <paramref name="count"/> items, seeding every size to <paramref name="defaultValue"/>.</summary>
    ...
    /// <summary>Gets the currently stored size for <paramref name="index"/>.</summary>
    public double GetSize(int index) => _values[index];

    /// <summary>Sets the size of <paramref name="index"/> to <paramref name="value"/>; returns the delta applied.</summary>
    public double SetSize(int index, double value)
    {
        double delta = value - _values[index];
        if (delta == 0d)
        {
            return 0d;
        }
        ...
        for (int i = index + 1; i <= _count; i += i & -i)
        {
            _tree[i] += delta;
        }
